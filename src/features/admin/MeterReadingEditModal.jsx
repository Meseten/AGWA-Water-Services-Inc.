// src/features/admin/MeterReadingEditModal.jsx
...
const MeterReadingEditModal = ({ isOpen, onClose, onSave, reading, isSaving }) => {
    const [formData, setFormData] = useState({});

    useEffect(() => {
        if (reading) {
            // Firestore timestamps need to be converted to a string format for the date input
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Edit Reading for ${reading.accountNumber}`}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <input type="number" name="readingValue" value={formData.readingValue} onChange={handleChange} className={commonInputClass} step="0.01" required />
                </div>
                <div>
                    ...
                    <input type="date" name="readingDate" value={formData.readingDate} onChange={handleChange} className={commonInputClass} required />
                </div>
                <div>
                    ...
                    <select name="readingType" value={formData.readingType} onChange={handleChange} className={commonInputClass}>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" value={formData.notes} onChange={handleChange} className={commonInputClass} />
                </div>
